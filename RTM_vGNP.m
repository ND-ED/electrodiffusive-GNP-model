...
tao=Y(7);dA=Y(8);gamma=Y(9);g_KV=Y(10);g_NaV=Y(11);type=Y(12);
...
if type==1
    ...
    r_Kbar=Kbart*96485*(100/26.64)*(1/(Z(1,1)*V^3+Z(1,2)*V^2+Z(1,3)*V+Z(1,4)));
    r_Nabar=Nabart*96485*(100/26.64)*(1/(Z(2,1)*V^3+Z(2,2)*V^2+Z(2,3)*V+Z(2,4)));
elseif type==2
    Kbart=(c_K_i-c_K_o*exp(-V/26.64)).*(1./(1-exp(-V/26.64))).*(V./(V-26.64*log(c_K_o/c_K_i)));
    r_Kbar=Kbart*96485*(100/26.64)*(1/(Z(1,1)*V^3+Z(1,2)*V^2+Z(1,3)*V+Z(1,4)));
    r_Nabar=1;
...
g_Na=g_NaV*r_Nabar;g_K=g_KV*r_Kbar;

dnavt=-gamma*g_Na*(m^3)*h*(V-26.64*log(c_Na_o/c_Na_i));dkvt=gamma*g_K*(n^4)*(V-26.64*log(c_K_o/c_K_i));
