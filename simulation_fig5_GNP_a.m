...
l_t=5000;dt=0.1;%fig. 5a: l_t=5000; fig. 5d: l_t=200000;
stepc=1;t_tsee=stepc:stepc:l_t;l_tc=length(t_tsee);
X_inp=XX3;dA=-110;

X_reco1=zeros(l_tc,7);X_RTM=zeros(1,6);A_reco=zeros(l_tc,1);Y_RTM=zeros(1,12);
...
for i2=1:1
    ...
    for i1=1:l_t
        ...
        Y_RTM(:)=[P_KL,P_NaL,P_ClL,c_K_t,c_Na_t,c_Cl_t,tao,I_pump,dA,gamma,g_KV,g_NaV];
        k1=RTM_GNP_a(X_RTM,Y_RTM,G_RTM);k2=RTM_GNP_a(X_RTM+(dt/2)*k1,Y_RTM,G_RTM);
        k3=RTM_GNP_a(X_RTM+(dt/2)*k2,Y_RTM,G_RTM);k4=RTM_GNP_a(X_RTM+dt*k3,Y_RTM,G_RTM);
