...
X_inp=[V,m,h,n];P_KL=2*10^(-8);P_NaL=1*10^(-9);P_ClL=4*10^(-9);
c_L_i=c_K_i+(P_NaL/P_KL)*c_Na_i+(P_ClL/P_KL)*c_Cl_o;
c_L_o=c_K_o+(P_NaL/P_KL)*c_Na_o+(P_ClL/P_KL)*c_Cl_i;
...
Lbar0=(c_L_i-c_L_o*exp(-V/26.64))*(1/(1-exp(-V/26.64)))*(V/(V-26.64*log(c_L_o/c_L_i)));
G_KL=F*(100/26.64)*P_KL*Kbar0;G_NaL=F*(100/26.64)*P_NaL*Nabar0;G_ClL=F*(100/26.64)*P_ClL*Clbar0;
G_L=G_KL+G_NaL+G_ClL;E_L=(G_KL*E_K+G_NaL*E_Na+G_ClL*E_Cl)/(G_KL+G_NaL+G_ClL);
I_pump=0.048;G_glu=0.12;E_glu=0;

l_t=1000;dt=0.1;
stepc=1;t_tsee=stepc:stepc:l_t;l_tc=length(t_tsee);

X_recoh=zeros(l_tc,4);X_RTM=zeros(1,4);Y_RTM=zeros(1,10);
...
for i2=1:1
    ...
    for i1=1:l_t
        ...
        Y_RTM(:)=[g_K,g_Na,E_K,E_Na,gamma,I_pump,Glu*G_glu,E_glu,G_L,E_L];

        k1=RTM_hh(X_RTM,Y_RTM);k2=RTM_hh(X_RTM+(dt/2)*k1,Y_RTM);
        k3=RTM_hh(X_RTM+(dt/2)*k2,Y_RTM);k4=RTM_hh(X_RTM+dt*k3,Y_RTM);
