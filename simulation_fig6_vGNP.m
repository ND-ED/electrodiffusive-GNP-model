...
I_pump=0.048;P_KV=20;P_NaV=30;
l_t=1500000;dt=0.1;
stepc=1;t_tsee=stepc:stepc:l_t;l_tc=length(t_tsee);
X_inp=XX3;dA=-110;

X_reco2=zeros(l_tc,7);X_reco3=zeros(l_tc,7);X_reco4=zeros(l_tc,7);
X_RTM=zeros(1,6);Y_RTM=zeros(1,12);
Z_RTM=[cokk(1),cokk(2),cokk(3),cokk(4);conn(1),conn(2),conn(3),conn(4)];
...
for i2=1:3
    ...
    for i1=1:l_t
        ...
        Y_RTM(:)=[P_KL,P_NaL,P_ClL,c_K_t,c_Na_t,c_Cl_t,tao,dA,gamma,P_KV,P_NaV,i2];
        k1=RTM_vGNP(X_RTM,Y_RTM,Z_RTM,G_RTM);k2=RTM_vGNP(X_RTM+(dt/2)*k1,Y_RTM,Z_RTM,G_RTM);
        k3=RTM_vGNP(X_RTM+(dt/2)*k2,Y_RTM,Z_RTM,G_RTM);k4=RTM_vGNP(X_RTM+dt*k3,Y_RTM,Z_RTM,G_RTM);
        ...
        if rem(i1,stepc)==0
            ii1=i1/stepc;
            if i2==1
                X_reco2(ii1,2:7)=X_RTM;
            elseif i2==2
                X_reco3(ii1,2:7)=X_RTM;
            else
                X_reco4(ii1,2:7)=X_RTM;
            end
        end
